...
class PostgresQueryGenerator extends AbstractQueryGenerator {
  ...
  createTableQuery(tableName, attributes, options) {
    ...
    if (options.partition && options.partition.attribute) {
      partition = ` PARTITION BY ${options.partition.attribute} `;
    }
    ...
    const tableNameRaw = [`CREATE TABLE ${databaseVersion === 0 || semver.gte(databaseVersion, '9.1.0') ? 'IF NOT EXISTS ' : ''}${quotedTable} (${attributesClause})${partition}${comments}${columnComments};`]

    if (options.partition && options.partition.extra) {
      for (const extra of (Array.isArray(options.partition.extra) ? options.partition.extra : [options.partition.extra])) {
        const partitionTable = this.quoteTable(`${tableName}_${extra.table}`)
        tableNameRaw.push(
          `CREATE TABLE ${databaseVersion === 0 || semver.gte(databaseVersion, '9.1.0') ? 'IF NOT EXISTS ' : ''}${partitionTable} partition of ${quotedTable} for values from ('${extra.from}') to ('${extra.to}');`
        )
      }
    }

    console.log('tableNameRaw:', tableNameRaw.join(' '));
    return tableNameRaw.join(' ');
  }
